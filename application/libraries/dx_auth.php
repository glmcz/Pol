...
class DX_Auth
{
	...
	*/
	...
	{
		return $this->ci->session->userdata('DX_user_image');
	}
	...
	function is_username_available($username, $old_username = false)
	{
		// Load Models
		$this->ci->load->model('dx_auth/users', 'users');
		$this->ci->load->model('dx_auth/user_temp', 'user_temp');

		$users = $this->ci->users->check_username($username, $old_username);
		$temp = $this->ci->user_temp->check_username($username, $old_username);
	...
	function is_email_available($email, $old_email = false)
	{
		// Load Models
		$this->ci->load->model('dx_auth/users', 'users');
		$this->ci->load->model('dx_auth/user_temp', 'user_temp');

		$users = $this->ci->users->check_email($email, $old_email);
		$temp = $this->ci->user_temp->check_email($email, $old_email);
	...
	function register($username, $password, $email)
	{
		...
		$new_user = array(
			'username'				=> $username,
			'password'				=> crypt($this->_encode($password),$this->ci->config->item('DX_salt')),
			'email'						=> $email,
    ...
    function update_user($user_id, $username, $email, $password = FALSE, $image = FALSE)
	{
		$result = FALSE;
		// Load Models
		$this->ci->load->model('dx_auth/users', 'users');
		...
		// Update user
		$result = $this->ci->users->set_user($user_id, $user);
		...
		if ($login)
		{
			...
			if ($query = $this->ci->users->get_login($login) AND $query->num_rows() == 1)
			{
				...
				if ( ! $row->newpass_key)
				{
					...
					$encode = crypt($this->_encode($data['password']),$this->ci->config->item('DX_salt'));
	...
	function change_password($old_pass, $new_pass)
	{
		...
		if ($query = $this->ci->users->get_user_by_id($this->ci->session->userdata('DX_user_id')) AND $query->num_rows() > 0)
		{
			...
			if (crypt($pass, $row->password) === $row->password)
			{
				...
				$new_pass = crypt($this->_encode($new_pass),$this->ci->config->item('DX_salt'));
