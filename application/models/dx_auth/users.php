...
class Users extends CI_Model
{
	...
	function delete_user_image($image)
	{
		if(!empty($image))
		{
			$file = 'assets/uploads/avatars/'.$image ;
	...
	{
		$this->db->where('user_add', $user_id);
		return $this->db->get('materials');
	}

    function get_user_updated_articles($user_id)
	{
		$this->db->where('user_update', $user_id);
		return $this->db->get('materials');
	}

    function get_user_comments($user_email, $user_id = FALSE)
	{
		//$this->db->where('user_update', $user_id);
...
//		users.id as user_id
        ...
        $this->db->where('comments.email', $user_email);
		if($user_id){
			$this->db->or_where('comments.uid', $user_id);
		}
        $this->db->join('materials', 'materials.material_id = comments.foreign_id');
